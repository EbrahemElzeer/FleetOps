...
public sealed class MarkOrderAsPickedUpEndpointTests: IClassFixture<CustomWebApplicationFactory>
{
    private readonly HttpClient _client;

    public MarkOrderAsPickedUpEndpointTests(
    ...
    [Fact]
    public async Task MarkAsPickedUp_WhenOrderDoesNotExist_ShouldReturnNotFound()
    {
        // Arrange
    ...
    [Fact]
    public async Task MarkAsPickedUp_WhenOrderIsDriverAccepted_ShouldMarkAsPickedUpSuccessfully()
    {
        // Arrange
        ...
        // Act
        var response = await _client.PutAsJsonAsync(
            $"/api/orders/{createdOrder.Id}/pickup",
            request);
    ...
    [Fact]
    public async Task MarkAsPickedUp_WhenDriverIdIsEmpty_ShouldReturnBadRequest()
    {
        // Arrange
        var createdOrder = await CreateOrderAsync();
        var driverId = Guid.NewGuid();
    ...
    [Fact]
    public async Task MarkAsPickedUp_WhenDriverIsNotAssignedDriver_ShouldReturnForbidden()
    {
        // Arrange
        var createdOrder = await CreateOrderAsync();
    ...
    [Fact]
    public async Task MarkAsPickedUp_WhenOrderIsPending_ShouldReturnConflict()
    {
        // Arrange
        var createdOrder = await CreateOrderAsync();
    ...
    [Fact]
    public async Task MarkAsPickedUp_WhenOrderIsAssignedButNotAccepted_ShouldReturnConflict()
    {
        // Arrange
        var createdOrder = await CreateOrderAsync();
        var driverId = Guid.NewGuid();
    ...
    [Fact]
    public async Task MarkAsPickedUp_WhenOrderIsAlreadyPickedUp_ShouldReturnConflict()
    {
        // Arrange
        var createdOrder = await CreateOrderAsync();
        var driverId = Guid.NewGuid();
        ...
        var firstRequest = new MarkOrderAsPickedUpRequest(
            DriverId: driverId);
        ...
        var secondRequest = new MarkOrderAsPickedUpRequest(
            DriverId: driverId);
