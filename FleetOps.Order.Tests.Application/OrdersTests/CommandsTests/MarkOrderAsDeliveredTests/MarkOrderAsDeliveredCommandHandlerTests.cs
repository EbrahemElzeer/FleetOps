...
namespace FleetOps.Order.Tests.Application.OrdersTests.CommandsTests.MarkOrderAsDeliveredTests;

public class MarkOrderAsDeliveredCommandHandlerTests
{
    ...
    public MarkOrderAsDeliveredCommandHandlerTests()
    {
        ...
        _handler = new MarkOrderAsDeliveredCommandHandler(
            _unitOfWorkMock.Object,
            _orderRepositoryMock.Object);
    }

    [Fact]
    public async Task Handle_WhenOrderIsPickedUp_ShouldMarkOrderAsDelivered()
    {
        // Arrange
        var driverId = Guid.NewGuid();
        var order = CreatePickedUpOrder(driverId);

        var command = new MarkOrderAsDeliveredCommand(
            order.Id,
            driverId);
    ...
    public async Task Handle_WhenOrderIsNotPickedUp_ShouldReturnFailureWithoutSaving()
    {
        ...
        result.IsFailure.Should().BeTrue();
        result.Errors.Should().Contain(
            OrderErrors.CannotMarkAsDelivered(
